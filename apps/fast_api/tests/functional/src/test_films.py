...
async def call_films(params: dict) -> tuple[int, list | dict]:
    async with (
        ...
        session.get(f"{settings.api.url}/api/v1/films", params=params) as resp,
    ):
...
def build_films_cache_key(
    offset: int = 0, limit: int = 50, sort: str | None = None, genre: str | None = None
) -> str:
...
@pytest.mark.asyncio
async def test_films_pagination_and_defaults(
    pagination_test_data, query_params, expected_length
):
...
async def test_films_filter_and_specific_retrieval(generate_movies, es_write_data):
    ...
    drama_movies = [
        generate_movies(
            ...
            title="Drama One",
            rating=8.0,
            genres=[{"uuid": genre_uuid_2, "name": "Drama"}],
        )[0],
        generate_movies(
            count=1,
            title="Drama Two",
    ...
    s, b = await call_films(
        {"genre": genre_uuid_2, "sort": "imdb_rating", "page_size": 10}
    )
    assert len(b) == 2
...
async def test_films_redis_cache(generate_movies, es_write_data, redis_client):
    ...
    assert await redis_client.exists(cache_key_diff), (
        "New cache key not created for different params"
    )
...
async def test_films_response_schema(generate_movies, es_write_data):
    ...
    for film in b:
        assert REQUIRED_FILM_FIELDS.issubset(film.keys()), (
            f"Missing fields: {REQUIRED_FILM_FIELDS - film.keys()}"
        )
