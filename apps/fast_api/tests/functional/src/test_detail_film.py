...
async def call_film_detail(film_id: str) -> tuple[int, dict | list | str]:
    async with (
        ...
        session.get(f"{settings.api.url}/api/v1/films/{film_id}") as resp,
    ):
...
@pytest.mark.asyncio
async def test_film_detail_validation(
    film_id, expected_status, generate_movies, es_write_data
):
...
async def test_film_detail_success(generate_movies, es_write_data):
    ...
    assert required_fields.issubset(film.keys()), (
        f"Missing: {required_fields - film.keys()}"
    )
...
async def test_film_detail_redis_cache(generate_movies, es_write_data, redis_client):
    ...
    assert await redis_client.exists(cache_key), (
        f"Cache key {cache_key} not found after first request"
    )
    ...
    assert await redis_client.exists(other_cache_key), (
        "New cache key not created for different film_id"
    )
    assert not await redis_client.exists(f"film:{film_uuid}_wrong"), (
        "Wrong key pattern detected"
    )
...
async def test_film_detail_idempotency(generate_movies, es_write_data):
    film_uuid = str(uuid.uuid4())
    movie = generate_movies(
        count=1, film_uuid=film_uuid, title="IdempotencyTest", rating=7.7
    )
    await es_write_data(settings.elastic.index, movie)
